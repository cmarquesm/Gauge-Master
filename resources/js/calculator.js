...
function renderTabla(afinacion, escala, nivel) {
    ...
    datos.cuerdas.forEach((cuerda, i) => {
        ...
        const opciones = notasDisponibles
            .map(
                (n) =>
                    `<option value="${n}" ${n === cuerda.nota ? "selected" : ""}>${n}</option>`,
            )
            .join("");

        tr.innerHTML = `
      <td class="border border-gray-800 p-2 text-center text-gray-500">${i + 1}</td>
      <td class="border border-gray-800 p-2 text-center">
        <select class="nota-select bg-black text-green-400 border border-gray-700 rounded px-1 py-0.5 text-xs focus:ring-1 focus:ring-green-500">${opciones}</select>
      </td>
      <td class="border border-gray-800 p-2 text-center">
        <input type="number" value="${cuerda.calibre}" step="0.001" min="0.005" max="0.100" class="calibre-input bg-black text-green-400 border border-gray-700 rounded px-1 py-0.5 w-20 text-center text-xs focus:ring-1 focus:ring-green-500">
      </td>
      <td class="border border-gray-800 p-2 text-center font-bold text-green-500" data-tension>${cuerda.tension_lb}</td>
    `;
...
function actualizarTensionTotal() {
    ...
    const total = Array.from(tensiones).reduce(
        (s, td) => s + parseFloat(td.textContent || 0),
        0,
    );
